...
public class AdminRejectRequest implements IMessageHandler {
    ...
    public void handle(WebSocket conn, Map<String, Object> data, Gson gson, ServerContext context) {
        ...
        if (adminEmail == null) {
            System.err.println("[AdminRejectRequest] Từ chối: Thao tác từ một kết nối chưa đăng nhập!");
        ...
        if (currentRequester == null || !"ADMIN".equalsIgnoreCase(currentRequester.getRole())) {
            System.err.println("[AdminRejectRequest] Cảnh báo: Tài khoản " + adminEmail + " cố tình hack quyền từ chối Admin!");
            responseMap.put("status", "ERROR");
        ...
        if (userId == null || userId.trim().isEmpty()) {
            System.err.println("[AdminRejectRequest] Lỗi: Không tìm thấy hoặc ID cần từ chối bị trống!");
            responseMap.put("status", "ERROR");
        ...
        if (isUpdated) {
            System.out.println("[AdminRejectRequest] Thành công: Admin [" + adminEmail + "] đã từ chối User [" + userId + "] và đặt role thành BIDDER.");

            responseMap.put("status", "SUCCESS");
            responseMap.put("userId", userId);
            responseMap.put("message", "Đã từ chối quyền Admin và đặt thành tài khoản BIDDER!");
        } else {
            System.err.println("[AdminRejectRequest] Thất bại: Không thể cập nhật database cho User [" + userId + "].");
