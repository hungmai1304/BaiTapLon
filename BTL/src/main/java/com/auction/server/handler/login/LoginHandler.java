...
public class LoginHandler implements IMessageHandler {
    ...
    public void handle(WebSocket conn, Map<String, Object> data, Gson gson, ServerContext context) {
        LOGGER.info("[LoginHandler] Đang xử lý đăng nhập...");
        ...
        try {
            ...
            if (email != null && email.toLowerCase().endsWith("@admin.com")) {
                LOGGER.info("[LoginHandler] Phát hiện định dạng Email Admin. Tiến hành xác thực...");
                ...
                if (maybeAdmin instanceof Admin) {
                    ...
                    LOGGER.info("[LoginHandler] Admin [" + admin.getUsername() + "] đã vào hệ thống.");
                    return;
                } else {
                    LOGGER.info("[LoginHandler] Xác thực Admin thất bại!");
                    Response response = new Response(
                            MessageType.ADMIN_LOGIN_RESPONSE,
                            "ERROR",
            ...
            if (userCheck != null && "BANNED".equalsIgnoreCase(userCheck.getStatus())) {
                LOGGER.severe("[LoginHandler] Từ chối xác thực: Tài khoản [" + email + "] đang bị khóa (BANNED)!");
                Response response = new Response(
            ...
            if (loginUser != null) {
                ...
                if ("BLACKLIST".equalsIgnoreCase(loginUser.getStatus())) {
                    ...
                    LOGGER.info("[LoginHandler] Cảnh báo: Thành viên nhóm danh sách đen [" + loginUser.getEmail() + "] vừa đăng nhập.");
                }
                ...
                conn.send(gson.toJson(response));
                LOGGER.info("[LoginHandler] User [" + loginUser.getUsername() + "] với vai trò [" + loginUser.getRole() + "] và trạng thái [" + loginUser.getStatus() + "] đã vào hệ thống.");
